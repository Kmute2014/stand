...
export const EditResponseModal: React.FC = () => {
  const { editingResponse, setEditingResponse, updateResponse, submitStandup, currentUser, responses } = useAppContext();
  ...
  const isEditing = !!editingResponse.id;
  ...
  const handleSubmit = () => {
    ...
    if (!isEditing) {
      // Check if user already has a response for the selected date
      ...
      const existingResponse = responses.find(r =>
        r.userId === currentUser?.id && r.date === formattedDate
      );

      if (existingResponse) {
        alert(`You have already submitted a standup response for ${formattedDate}. You can edit your existing response instead.`);
        return;
      }
    }
    ...
    <div className="modal-bg show">
      <div className="modal" style={{ width: '600px' }}>
        <div className="modal-h">
          <div className="modal-title">{isEditing ? 'Edit Standup Response' : 'New Standup Response'}</div>
          <div className="modal-close" onClick={handleClose}>×</div>
        </div>
        <div className="modal-body">

          {/* Date field - only show for new responses */}
          {!isEditing && (
            <div className="form-group">
              <label className="form-label text-[10px] uppercase tracking-widest text-[var(--text-3)] mb-2">Standup Date</label>
              <input
                type="date"
        ...
        <div className="modal-foot">
          ...
          <button className="btn btn-primary" onClick={handleSubmit}>{isEditing ? 'Update response' : 'Submit response'}</button>
        </div>
      </div>
    </div>
  );
