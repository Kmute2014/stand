...
export const EditProjectModal: React.FC<EditProjectModalProps> = () => {
  const { showToast, currentUser, users, programs, projects, updateProject } = useAppContext();
  ...
  // Check if current user has permission to edit projects
  const canEditProject = currentUser?.role === 'Admin' || currentUser?.role === 'Project Manager/Scrum Master';

  // Load project data when modal opens
  useEffect(() => {
    const modal = document.getElementById('modal-edit-project') as HTMLElement;
    if (modal?.classList.contains('show')) {
      // Find the project being edited from the projects array
      ...
      if (projectToEdit) {
        ...
        setSelectedOwnerId(projectToEdit.owner?.id || '');
        setSelectedTeamMembers(projectToEdit.teamMembers?.map(m => m.id) || []);
        setSelectedProgramId(projectToEdit.programId || '');
      }
    }
  ...
  const handleUpdate = async () => {
    if (!canEditProject) {
      showToast('Only admins and project managers can edit projects.', 'red');
      return;
    }

    if (!editingProjectId) {
      showToast('No project selected for editing.', 'red');
      return;
    }
    ...
    try {
      ...
      console.log(`Project "${name}" updated in Firestore with ID: ${editingProjectId}`);
      showToast(`Project "${name}" updated successfully!`, 'green');
      ...
      showToast(`Failed to update project: ${errorMsg}`, 'red');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const openEditModal = (projectId: string) => {
    ...
    document.getElementById('modal-edit-project')?.classList.add('show');
  };

  // Make this function available globally
  useEffect(() => {
    (window as any).openEditProjectModal = openEditModal;
    return () => {
      delete (window as any).openEditProjectModal;
    };
  }, []);
  ...
  if (!currentUser || !users || !programs) {
    return (
      <div className="modal-bg" id="modal-edit-project">
        <div className="modal">
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          {!canEditProject && (
            <div className="mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded text-sm">
              Only admins and project managers can edit projects. Please contact an admin.
            </div>
          )}
          ...
          <div className="form-group">
            ...
            <input
              ...
              disabled={!canEditProject || isLoading}
            />
          </div>
        ...
        <div className="modal-foot">
          ...
          <button
            ...
            disabled={isLoading || !canEditProject || !name.trim() || !startDate || !endDate || !selectedOwnerId || !selectedProgramId || !editingProjectId}
          >
